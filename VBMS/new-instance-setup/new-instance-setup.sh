...
vault kv put secret/platform/candp-dbas/$DATABASE endpoint=$HOST port=$PORT sid=$SID master-password=$MASTER_PASSWORD master-user=$MASTER DBSNMP=$DBSNMP_PASSWORD BIP_DBA=$BIPDBA_PASSWORD LIQUIBASE_ADMIN=$LIQUID_PASSWORD

if [[ "$AWSACCOUNT" != "P" ]]; then
    echo "Executing non-prod baseline script."

    CPASA_PASSWORD="$(generate_password)"

    sqlplus $MASTER/"$MASTER_PASSWORD"@$HOST:$PORT/$SID <<EOF
start new-instance-setup-nonprod $CPASA_PASSWORD
exit
EOF

    vault kv put secret/platform/candp-asa/$DATABASE endpoint=$HOST port=$PORT sid=$SID CP_ASA=$CPASA_PASSWORD
